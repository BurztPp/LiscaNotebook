...
class ContrastAdjuster:
    ...
    def _get_limits(self, img=None):
        # TODO: Fix min/max display for uint16 and float
        if self.scale_var.get() == 'NONE':
            if img is None:
                img = self.img
            if img is not None:
                if np.issubdtype(img.dtype, np.unsignedinteger):
                    ...
                    pmin = iinfo.min
                    pmax = iinfo.max
                elif np.issubdtype(img.dtype, 'bool'):
                    pmin = 0
                    pmax = 1
                else:
                    pmin = 0
    ...
    def draw_hist(self):
        ...
        #TODO: use global maximum of stack/channel
        if np.issubdtype(self.img.dtype, np.unsignedinteger):
            ...
            elif self.img_max <= 0xffff:
                self.hist_max = 0xffff      # 16-bit
            elif self.img_max <= 0xffffffff:
                self.hist_max = 0xffffffff  # 32-bit
            else:
                self.hist_max = 2**64 - 1   # 64-bit
        elif np.issubdtype(self.img.dtype, 'bool'):
            self.hist_max = 1
        else:
            self.hist_max = self.img_max
